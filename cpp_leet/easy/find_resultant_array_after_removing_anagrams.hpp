...
class Solution {
  private:
    void inline zeroize_frequency(size_t* freq);
    void inline populate_frequency(size_t* freq, const std::string& word);
    bool inline are_frequencies_same(size_t* freq_l, size_t* freq_r);
  public:
    std::vector<std::string> removeAnagrams(std::vector<std::string>& words);
};

void inline Solution::zeroize_frequency(size_t* freq) {
  for (size_t i = 0ul; i < 26ul; ++i) {
    freq[i] ^= freq[i];
  }
}

void inline Solution::populate_frequency(size_t* freq, const std::string& word) {
...
bool inline Solution::are_frequencies_same(size_t* freq_l, size_t* freq_r) {
  for (size_t i = 0ul; i < 26ul; ++i) {
